"""
╔══════════════════════════════════════════════════════════════╗
║         EVALON WINNERS — TELEGRAM SUPPORT BOT v3.0          ║
║                                                              ║
║  Features:                                                   ║
║  ✅ Delete-then-Send (messages "melt away")                  ║
║  ✅ 24-min cooldown → Start button reminder                  ║
║  ✅ Join channel check before any service                    ║
║  ✅ Contact Support — NO username shown to user              ║
║  ✅ 10+ random replies per service (truly random)            ║
...
BOT_TOKEN         = "8663696851:AAFCjW_0GtIIq-N6fw1xCYMYDE550nwr0Uo"
...
COOLDOWN_MINUTES  = 24 * 60   # 24 hours in minutes
...
pending_requests: dict = {}   # uid → {chat_id, chat_title, user, time}
reply_map: dict        = {}   # admin_msg_id → user_id
...
# ══════════════════════════════════════════════════════════════
#  DELETE HELPER — silently ignore if already deleted
# ══════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════
#  TRACK & DELETE LAST BOT MESSAGE (melt effect)
# ══════════════════════════════════════════════════════════════

async def send_and_track(context, chat_id: int, user_data: dict, **kwargs):
    """Send a message, delete the previous one → melt effect."""
    # Delete previous bot message
    old_id = user_data.get("last_bot_msg_id")
    if old_id:
        await safe_delete(context, chat_id, old_id)

    # Send new message
    msg = await context.bot.send_message(chat_id=chat_id, **kwargs)
    user_data["last_bot_msg_id"] = msg.message_id

    # Reset cooldown timer
    user_data["last_activity"] = datetime.now().isoformat()
    return msg

async def send_photo_and_track(context, chat_id: int, user_data: dict, **kwargs):
    """Send a photo, delete the previous one → melt effect."""
    old_id = user_data.get("last_bot_msg_id")
    if old_id:
        await safe_delete(context, chat_id, old_id)

    msg = await context.bot.send_photo(chat_id=chat_id, **kwargs)
    user_data["last_bot_msg_id"] = msg.message_id
    user_data["last_activity"] = datetime.now().isoformat()
    return msg
...
IMG_SIGNALS   = "https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?w=800&q=80"
IMG_SOCIAL    = "https://images.unsplash.com/photo-1642790106117-e829e14a795f?w=800&q=80"
IMG_INDICATOR = "https://images.unsplash.com/photo-1590283603385-17ffb3a7f29f?w=800&q=80"
IMG_AUTOBOT   = "https://images.unsplash.com/photo-1518186285589-2f7649de83e0?w=800&q=80"
...
async def notify_admins(context, user, event: str):
...
async def send_cooldown_reminder(context: ContextTypes.DEFAULT_TYPE):
    """Job that fires after COOLDOWN_MINUTES of inactivity."""
...
def schedule_cooldown(context, chat_id, name, lang):
    """Remove old cooldown job and schedule a new one."""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    register_user(user)
    await notify_admins(context, user, "Started bot")

    # Delete previous bot message (melt effect)
    ...
    msg = await context.bot.send_message(
        chat_id=cid,
        text=random.choice(WELCOME.get(lang, WELCOME["en"])).format(
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # All other callbacks — delete previous, send new (melt effect)
    await safe_delete(context, cid, query.message.message_id)
    await typing_action(cid, context, 1.5)
    ...
    # ── Services ──────────────────────────────────────────────
        ...
        await notify_admins(context, user, "Viewed VIP Signals")
        replies = get_replies(SIGNALS_REPLIES, lang)
        msg = await context.bot.send_photo(
            chat_id=cid, photo=IMG_SIGNALS,
        ...
        await notify_admins(context, user, "Viewed Social Trading")
        replies = get_replies(SOCIAL_REPLIES, lang)
        msg = await context.bot.send_photo(
            chat_id=cid, photo=IMG_SOCIAL,
            caption=random.choice(replies),
            parse_mode="Markdown", reply_markup=svc_keyboard(lang),
        )
        context.user_data["last_bot_msg_id"] = msg.message_id
        ...
        await notify_admins(context, user, "Viewed Indicator")
        replies = get_replies(INDICATOR_REPLIES, lang)
        msg = await context.bot.send_photo(
            chat_id=cid, photo=IMG_INDICATOR,
            caption=random.choice(replies),
        ...
        await notify_admins(context, user, "Viewed Auto Bot")
        replies = get_replies(AUTOBOT_REPLIES, lang)
        msg = await context.bot.send_photo(
            chat_id=cid, photo=IMG_AUTOBOT,
            caption=random.choice(replies),
            parse_mode="Markdown", reply_markup=svc_keyboard(lang),
        )
        context.user_data["last_bot_msg_id"] = msg.message_id

    # ── Support — NO username shown ───────────────────────────
    elif data == "do_support":
        await notify_admins(context, user, "Requested support via button")
        msg = await context.bot.send_message(
            chat_id=cid,
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user.id) and message.reply_to_message:
        if reply_map.get(message.reply_to_message.message_id):
            await handle_admin_reply(update, context)
            return

    # Admin non-reply → skip
    if is_admin(user.id):
        return

    register_user(user)

    # Media from user → forward to admin
    ...
    # ── Keyword routing ───────────────────────────────────────
    if any(w in low for w in ["hi","hello","hey","start","menu","hujambo","habari",
                               "salaam","bonjour","hola","привет","مرحبا","नमस्ते",
                               "こんにちは","olá","hallo","salam"]):
        msg = await context.bot.send_message(
            chat_id=cid,
            text=random.choice(WELCOME.get(lang, WELCOME["en"])).format(
                name=user.first_name, business=BUSINESS_NAME),
    ...
    elif any(w in low for w in ["signal","vip","alert"]):
        msg = await context.bot.send_photo(
            chat_id=cid, photo=IMG_SIGNALS,
            caption=random.choice(get_replies(SIGNALS_REPLIES, lang)),
            parse_mode="Markdown", reply_markup=svc_keyboard(lang))
        context.user_data["last_bot_msg_id"] = msg.message_id

    elif any(w in low for w in ["social","copy","pocket","copy trade"]):
        msg = await context.bot.send_photo(
            chat_id=cid, photo=IMG_SOCIAL,
            caption=random.choice(get_replies(SOCIAL_REPLIES, lang)),
            parse_mode="Markdown", reply_markup=svc_keyboard(lang))
        context.user_data["last_bot_msg_id"] = msg.message_id

    elif any(w in low for w in ["indicator","chart","mt4","mt5"]):
        msg = await context.bot.send_photo(
            chat_id=cid, photo=IMG_INDICATOR,
    ...
    elif any(w in low for w in ["auto","bot","robot","automatic"]):
        msg = await context.bot.send_photo(
            chat_id=cid, photo=IMG_AUTOBOT,
            caption=random.choice(get_replies(AUTOBOT_REPLIES, lang)),
            parse_mode="Markdown", reply_markup=svc_keyboard(lang))
        context.user_data["last_bot_msg_id"] = msg.message_id

    elif any(w in low for w in ["price","cost","fee","how much","bei","pesa","pay"]):
        await notify_admins(context, user, f"Price inquiry: {text}")
        msg = await context.bot.send_message(
            chat_id=cid,
    ...
    elif any(w in low for w in ["thank","thanks","asante","merci","gracias","спасибо","شكرا"]):
        msg = await context.bot.send_message(
            chat_id=cid,
...
def main():
    ...
    print(f"✅ {BUSINESS_NAME} Bot v3.0 is LIVE!")
    print("📋 Commands: /broadcast  /stats")
